...
def RegisterCustomer(request):
    ...
    if result_customer.exists() or result_owner.exists() or result_manager.exists():
        ...
        customer = Customer(customer_firstname=customer_firstname, customer_lastname=customer_lastname,
                            customer_dob=customer_dob, customer_gender=customer_gender,
...
def CheckAvailability(request, Worker_work_profile_id):
    ...
    if BookWorker_Date_of_Booking < date.today():
        ...
        return render(request, 'showdetails_loggedin.html',
                      {'Incorrect_dates': Incorrect_dates, 'worker': worker, 'customer': customer})
    ...
    for rv in bookworker:
        ...
        if (
            ...
            if rv.isAvailable:
                ...
                return render(request, 'showdetails_loggedin.html',
                              {'Message': Message, 'Available': Available, 'worker': worker, 'customer': customer,
                               'book_data': book_data})

            NotAvailable = True
            return render(request, 'showdetails_loggedin.html',
                          {'NotAvailable': NotAvailable, 'dates': rv, 'worker': worker, 'customer': customer})
    ...
    Available = True
    return render(request, 'showdetails_loggedin.html',
                  {'Available': Available, 'worker': worker, 'customer': customer, 'book_data': book_data})
...
def SentRequests(request):
    ...
    if bookworker.exists():
        ...
        return render(request, 'SentRequests.html',
                      {'customer': customer, 'bookworker': bookworker, 'Message': Message})
